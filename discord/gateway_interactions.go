...
func (h *gatewayEventHandler) InteractionCreate(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	case discordgo.InteractionMessageComponent:
		...
		case customID == "retry_create_round":
			slog.Info("✅ Calling HandleRetryCreateRound...")
			h.HandleRetryCreateRound(ctx, i)
		case strings.HasPrefix(customID, "round_accept|"):
			h.HandleRoundResponse(ctx, i, "accepted")
		case strings.HasPrefix(customID, "round_decline|"):
			h.HandleRoundResponse(ctx, i, "declined")
		case strings.HasPrefix(customID, "round_tentative|"):
			h.HandleRoundResponse(ctx, i, "tentative")
		}
...
func (h *gatewayEventHandler) HandleSignupButtonPress(ctx context.Context, i *discordgo.InteractionCreate) {
	...
	if err != nil {
		...
		h.interactionStore.Get(userID) //remove on error.
	} else {
