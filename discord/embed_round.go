...
func (h *gatewayEventHandler) SendRoundEventEmbed(channelID, eventID, title, description string, startTime time.Time, location, creatorID string) (*discordgo.Message, error) {
	// Convert time to Unix for Discord formatting
	unixTimestamp := startTime.Unix()

	// Fetch the user's information from the Discord API
	user, err := h.session.User(creatorID)
	if err != nil {
		return nil, err
	}

	// Get the user's name on the server
	guildID := h.config.Discord.GuildID
	member, err := h.session.GuildMember(guildID, creatorID)
	if err != nil {
		...
		return h.session.ChannelMessageSendComplex(channelID, messageSend)
	}
...
func (h *gatewayEventHandler) HandleRoundResponse(ctx context.Context, i *discordgo.InteractionCreate, response string) {
	user := i.Member.User
	...
	h.session.FollowupMessageCreate(i.Interaction, true, &discordgo.WebhookParams{
		Content: fmt.Sprintf("%s has %s the event!", user.Mention(), response),
	})
}

func (h *gatewayEventHandler) UpdateRoundEventEmbed(channelID, messageID string, acceptedParticipants, declinedParticipants, tentativeParticipants []roundtypes.RoundParticipant) error {
	...
	_, err := h.session.ChannelMessageEditEmbed(channelID, messageID, embed)
	return err
}
