...
func (bot *DiscordBot) Run(ctx context.Context) error {
	...
	bot.Logger.Info("Entering bot.Run()...")
	...
	if err != nil {
		bot.Logger.Error("Failed to register slash commands", attr.Error(err))
		return err
	}
	bot.Logger.Info("Slash commands registered successfully.")
	...
	if err != nil {
		bot.Logger.Error("Failed to initialize user module", attr.Error(err))
		return err
	}
	...
	err = leaderboard.InitializeLeaderboardModule(ctx, bot.Session, registry, bot.EventBus, bot.Logger, bot.Config, utils.NewEventUtil(), utils.NewHelper(bot.Logger), bot.InteractionStore)
	if err != nil {
		bot.Logger.Error("Failed to initialize leaderboard module", attr.Error(err))
		return err
	}
	...
	discordgoSession.AddHandler(func(s *discordgo.Session, r *discordgo.Ready) {
		...
		bot.Logger.Info("Discord bot is connected and ready.")
	})

	// Open Discord session
	err = bot.Session.Open()
	if err != nil {
		bot.Logger.Error("Error opening discord connection", attr.Error(err))
		return err
	}
...
func (b *DiscordBot) Close() {
	...
	if b.WatermillRouter != nil {
		if err := b.WatermillRouter.Close(); err != nil {
			b.Logger.Error("Failed to close Watermill router", attr.Error(err))
		}
	}
	// Close the Discord session.
	if err := b.Session.Close(); err != nil {
		b.Logger.Error("Failed to close Discord session", attr.Error(err))
	}
	// Close the EventBus.
	if err := b.EventBus.Close(); err != nil {
		b.Logger.Error("Failed to close EventBus", attr.Error(err))
	}
}
