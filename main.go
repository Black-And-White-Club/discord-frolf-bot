...
func main() {
	...
	eventUtils := utils.NewEventUtil()
	helper := utils.NewHelper(logger)
	// Create GatewayEventHandler
	gatewayHandler := discord.NewGatewayEventHandler(eventBus, logger, helper, cfg, discordSessionWrapper, operations)
	// Create domain routers (ensure they register handlers before running the router)
	userRouter := userrouter.NewUserRouter(logger, watermillRouter, eventBus, eventBus, operations, cfg, helper, tracerInstance)
	slogLogger.Info("Configuring user router subscribers...")
	if err := userRouter.Configure(userhandlers.NewUserHandlers(logger, cfg, eventUtils, helper, operations), eventBus); err != nil {
		log.Fatalf("Failed to configure user router: %v", err)
	}
	slogLogger.Info("User router subscribers configured.")
	// Create domain routers (ensure they register handlers before running the router)

	roundRouter := roundrouter.NewRoundRouter(logger, watermillRouter, eventBus, eventBus, discordSessionWrapper, cfg, tracerInstance)
	slogLogger.Info("Configuring round router subscribers...")
	if err := roundRouter.Configure(roundhandlers.NewRoundHandlers(logger, operations, cfg, eventUtils, gatewayHandler, helper, interactionStore), eventBus); err != nil {
		log.Fatalf("Failed to configure round router: %v", err)
	}
	slogLogger.Info("round router subscribers configured.")
	discordRouter := discordrouter.NewDiscordRouter(logger, watermillRouter, eventBus, eventBus, operations, cfg, helper, tracerInstance)
	slogLogger.Info("Configuring discord router subscribers...")
	if err := discordRouter.Configure(discordhandlers.NewDiscordHandlers(logger, cfg, eventUtils, helper, operations), eventBus); err != nil {
		log.Fatalf("Failed to configure discord router: %v", err)
	}
	slogLogger.Info("Discord router subscribers configured.")
